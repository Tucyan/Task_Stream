    ...
    <div className="h-full min-h-0 flex flex-col gap-6 max-w-7xl mx-auto w-full">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm opacity-60 dark:text-gray-400">管理您的所有任务与计划</p>
        </div>
        <div className="flex gap-3">
          ...
          <button onClick={onAddTask} className="bg-primary text-white px-4 py-2.5 rounded-xl shadow-lg shadow-primary/30 hover:brightness-110 transition-all flex items-center gap-2">
      ...
      <div className="bg-card rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-3 w-full md:w-auto">
          ...
          {(filterDateStart || filterDateEnd) && (
            <button onClick={clearDateFilter} className="text-xs text-red-400 hover:text-red-600">清除</button>
          )}
          {(!filterDateStart && !filterDateEnd) && (
            <span className="text-xs text-blue-400">显示所有任务</span>
          )}
        </div>
